 ...
 @section('page')
                 ...
                 <div class="row g-gs justify-content-center">
                     ...
                     @foreach ($plan as $planItem)
                         <div class="col-lg-4 col-md-6">
                             <div
                                 class="pricing {{ $planItem->name === 'Silver' ? 'pricing-featured bg-primary' : 'pricing' }}  ">
                                 <div class="pricing-body p-5">
                                     <div class="text-center">
                                         <h5 class="mb-3">{{ $planItem->name }}</h5>
                                         <h3 class="h2 mb-4">${{ $planItem->price }} <span class="caption-text text-muted">
                                                 / month</span></h3>
                                         <a href="{{ route('register') }}"
                                             class="btn {{ $planItem->name === 'Silver' ? 'btn-primary btn-block' : 'btn-primary btn-soft btn-block' }} ">Start
                                             free trial today</a>
                                     </div>
                                     <ul class="list gy-3 pt-4 ps-2">
                                         <li><em class="icon ni ni-check text-success"></em>
                                             <span><strong>{{ $planItem->monthly_word_limit }}</strong> Monthly Word
                                                 Limit</span></li>
                                         <li><em class="icon ni ni-check text-success"></em>
                                             <span><strong>{{ $planItem->templates }}+</strong> Templates</span></li>
                                         <li><em class="icon ni ni-check text-success"></em> <span>6+ Languages</span></li>
                                         <li><em class="icon ni ni-check text-success"></em> <span>Advance Editor
                                                 Tool</span></li>
     ...
     <section class="section section-bottom-0">
         <div class="container">
             ...
             @php
                 $questions = App\Models\Questions::orderBy('id', 'asc')->get();
             @endphp
                                 ...
                                 <div class="accordion-item">
                                     <h2 class="accordion-header">
                                         <button class="accordion-button {{ $index === 0 ? '' : 'collapsed' }}"
                                             data-bs-toggle="collapse" data-bs-target="#faq-1-{{ $index + 1 }}">
                                             {{ $question->title }} </button>
                                     </h2>
                                     <div id="faq-1-{{ $index + 1 }}"
