            ...
            <div class="col-md-3">
                <div class="card shadow-sm h-100">
                    ...
                    <div class="card-body p-0">
                        <a href="{{ route('chat-assistants.new', ['assistantId' => $assistant->id]) }}"
                            class="btn btn-primary w-100 rounded-0"> + New Conversation </a>
                        <div class="list-group list-group-flush">
                            @foreach ($conversations as $conv)
                                <a href="{{ route('chat-assistants.select', ['assistantId' => $assistant->id, 'conversationId' => $conv->conversation_id ?? $conv->id]) }}"
                                    class="list-group-item list-group-item-action {{ $selectedConversation &&
                                    ($selectedConversation->conversation_id ?? $selectedConversation->id) == ($conv->conversation_id ?? $conv->id)
                                        ? 'active'
                                        : '' }}">
                                    ...
                                    <p class="mb-1 text-truncate">
                                        {{ $conv->message ? substr($conv->message, 0, 20) . '...' : 'No messages Yet' }}</p>
                ...
                <div class="card shadow-sm h-100">
                    ...
                    <div class="card-body p-0">
                        <div class="chat-container" style="height: 500px; overflow-y:auto; padding: 15px;">
                            @if ($selectedConversation)
                                @foreach ($messages as $msg)
                                    <div
                                        class="chat-message {{ $msg->user_id == Auth::id() ? 'user-message' : 'assistant-message' }} mb-4 ">
                                        <div
                                            class="d-flex {{ $msg->user_id == Auth::id() ? 'justify-content-end' : 'justifuy-content-start' }} ">
                        ...
                        <div class="card-footer bg-white border-top">
                            <form action="{{ route('chat-assistants.send', ['assistantId' => $assistant->id]) }}"
                                method="post" class="input-group">
                                @csrf
                                <input type="text" name="message" class="form-control border-secondary"
